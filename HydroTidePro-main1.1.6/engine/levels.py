...
def compute_important_levels(time_array, height_array):
    """
    Compute seven important tide levels and their event counts from
    high-water and low-water peaks in an observed or predicted tide curve.
    ...
    if len(high_idx) == 0 or len(low_idx) == 0:
        raise ValueError("Not enough tide variation to detect high-water and low-water peaks.")
...
def format_levels_text(levels):
    """
    Format important levels as a compact text report.
    """
    labels = {
    ...
    for key in order:
        ...
        code = f"({key:<4})"
        lines.append(f"{label} {code} : {value:8.2f}, Event Count : {count:6d}")
    return "\n".join(lines)
